...
function sje_ud_profile_field_urlfilter($field) {
...
function sje_ud_user_created($field) {
 ...
 * 
 */
function sje_links($links, $attributes = array('class' => 'links')){
    /*
 ...
 * 
 */
function sje_textfield($element){
 ...
 * 
 */
function _sje_get_featured_carousel_array($field_array){
...
function sje_nd_location_address($field) {
 ...
 * 
 */
function sje_preprocess_page(&$variables){
  ...
  if (is_object($variables['node'])){
    ...
    if ($node->type == 'npage'){
      ...
      if ($node->field_page_subtype[0]['value'] == 'home'){
        ...
        $variables['featured_items'] = _sje_get_featured_carousel_array($node->field_featured_items);
      }
    }
 ...
 * 
 */
function sje_quicktabs_tabs($quicktabs, $active_tab = 'none') {
  $output = '';
 ...
 * 
 */
function sje_filter_tips_more_info() {
 ...
 * 
 */
function sje_preprocess_block(&$vars){
 ...
 */
function sje_pager($tags = array(), $limit = 10, $element = 0, $parameters = array(), $quantity = 9) {
 ...
 */
function sje_preprocess_comment_wrapper(&$vars) {
...
function sje_nd_location_gmap($field, $latitude, $longitude, $width, $height, $zoom, $autoclick = FALSE) {
  $map = array();
